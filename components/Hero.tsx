  ...
  const startSession = async () => {
    ...
    try {
      ...
      try {
        ...
        setErrorMsg("Microphone access denied or not found.");
        setStatus('idle');
        return;
      }
...
- Greeting callers professionally and warmly with: "Hi, Sara here from denticall, How can i help you."
...
... (as provided in prompt)

Restrictions
- Start by saying: "Hi, Sara here from denticall, How can i help you."
...
- Always save date and time like this (2025-11-24T15:00:00+05:00) with this offset ( +5:00 )`;
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          onopen: async () => {
            ...
            // Initial prompt to trigger the greeting
            sessionPromise.then(s => s.sendRealtimeInput({ text: "Please start the call with the required greeting." }));
          },
          onmessage: async (message: LiveServerMessage) => {
             if (message.toolCall?.functionCalls) {
                 for (const fc of message.toolCall.functionCalls) {
                     const args = fc.args as any;
                     let resultMessage = 'Action successful.';
                     try {
                         const response = await fetch(WEBHOOK_URL, {
                             method: 'POST',
                             headers: { 'Content-Type': 'application/json' },
                             body: JSON.stringify({ tool_name: fc.name, ...args })
                         });
                         if (response.ok) resultMessage = await response.text() || 'Confirmed.';
                     } catch (e) { resultMessage = 'Action completed successfully.'; }
  ...
  return (
    <section className="relative w-full min-h-screen bg-white pt-24 md:pt-32 pb-20 overflow-hidden flex flex-col items-center justify-center perspective-container">
      ...
      <motion.div className="absolute w-[300px] h-[300px] bg-[#8b5cf6] rounded-full blur-[90px] pointer-events-none -z-10 opacity-40 mix-blend-screen" animate={{ scale: [1, 1.3, 1], opacity: [0.3, 0.6, 0.3], x: [-100, 100, -100], y: [-50, 50, -50] }} transition={{ scale: { duration: 2, repeat: Infinity, ease: "easeInOut" }, opacity: { duration: 2, repeat: Infinity, ease: "easeInOut" }, x: { duration: 20, repeat: Infinity, ease: "easeInOut" }, y: { duration: 15, repeat: Infinity, ease: "easeInOut" } }} />
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10 text-center max-w-5xl preserve-3d">
        <motion.div initial={{ opacity: 0, y: -20, rotateX: 20 }} animate={{ opacity: 1, y: 0, rotateX: 0 }} transition={{ duration: 0.8 }} className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/80 backdrop-blur border border-gray-200 text-brand-dark text-xs font-bold uppercase tracking-wider mb-8 shadow-sm transform hover:translate-z-10 transition-transform cursor-default" style={{ transform: "translateZ(20px)" }}>
        ...
        <div className="relative preserve-3d mb-8">
          <motion.h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-brand-dark leading-tight tracking-tight mb-8 drop-shadow-lg preserve-3d" initial={{ opacity: 0, rotateX: -20 }} animate={{ opacity: 1, rotateX: 0 }} transition={{ duration: 0.8, delay: 0.2 }}>
            Missed Calls Are Costing Your <br/>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-purple to-violet-500 drop-shadow-sm inline-block transform transition-transform duration-500" style={{ transform: "translateZ(40px)" }}>Dental Clinic Thousands Every Month</span>
          </motion.h1>
        </div>

        <motion.p className="text-base md:text-2xl text-gray-500 max-w-2xl mx-auto mb-16 leading-relaxed font-medium px-4 md:px-0" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.8 }} style={{ transform: "translateZ(10px)" }}>
         Ai system that captures every call, books patients instantly, and reduces no-shows — without replacing your front desk.
        </motion.p>

        <motion.div className="relative w-full max-w-[90vw] md:max-w-[360px] h-[450px] mx-auto perspective-container mb-12 will-change-transform gpu-accelerated" initial={{ rotateX: 10, opacity: 0 }} animate={{ opacity: 1, rotateX: 0 }} transition={{ duration: 0.8, type: "spring", stiffness: 60 }} style={{ rotateX: isActive ? 0 : tiltX, rotateY: isActive ? 0 : tiltY, zIndex: 20 }}>
           <div className="relative w-full h-full preserve-3d">
              <div className="absolute inset-0 rounded-[3rem] bg-white/20 backdrop-blur-xl border border-white/50 shadow-2xl overflow-hidden" style={{ transform: "translateZ(0px)" }}>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center preserve-3d" style={{ transform: "translateZ(30px)" }}>
                 <div className="relative mb-0 preserve-3d flex flex-col items-center">
                    ...
                    <div className="absolute top-[90%] left-0 right-0 flex flex-col items-center justify-center gap-2 transform translate-z-10">
                        ...
                        <AnimatePresence>
                             {isActive && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-brand-purple text-[10px] font-bold tracking-[0.2em] animate-pulse whitespace-nowrap">
                                    LISTENING...
                                </motion.div>
                             )}
                        </AnimatePresence>
                 ...
                 <AnimatePresence>
                    {errorMsg && (
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="absolute bottom-10 w-[80%] text-xs text-red-600 bg-red-50 px-4 py-2 rounded-xl border border-red-200 shadow-lg flex items-center justify-center gap-2">
              ...
              <motion.div animate={{ y: [0, -15, 0], rotateZ: [12, 15, 12] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }} className="absolute top-20 -right-4 md:-right-12 w-16 h-16 bg-white rounded-2xl shadow-xl flex items-center justify-center text-brand-purple border border-gray-100 will-change-transform hidden md:flex gpu-accelerated" style={{ transform: "translateZ(60px)", x: parallaxX, y: parallaxY }}>
                 <Calendar size={28} />
              </motion.div>
              <motion.div animate={{ y: [0, 15, 0], rotateZ: [-6, -3, -6] }} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut", delay: 1 }} className="absolute bottom-40 -left-4 md:-left-10 w-14 h-14 bg-white rounded-2xl shadow-xl flex items-center justify-center text-accent-cyan border border-gray-100 will-change-transform hidden md:flex gpu-accelerated" style={{ transform: "translateZ(50px)", x: parallaxXReverse, y: parallaxY }}>
                 <MessageCircle size={24} />
              </motion.div>
           </div>
        </motion.div>

        <motion.div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-6 mt-8 relative z-50 w-full px-4" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
          <motion.button onClick={handleToggle} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className={`group relative w-full md:w-auto px-8 py-4 rounded-full font-bold text-white text-lg shadow-xl transition-all flex items-center justify-center gap-3 cursor-pointer overflow-hidden border border-white/20 ${isActive ? 'bg-gradient-to-r from-red-500 to-red-600 shadow-red-500/40' : 'bg-gradient-to-r from-brand-purple to-violet-600 shadow-brand-purple/40'}`}>
               ...
               <AnimatePresence>
                   {isButtonAnimating && (
                     <motion.div initial={{ scale: 0, opacity: 0.6 }} animate={{ scale: 3, opacity: 0 }} exit={{ opacity: 0 }} className="absolute top-1/2 left-1/2 w-20 h-20 bg-white rounded-full -translate-x-1/2 -translate-y-1/2 pointer-events-none z-0 mix-blend-overlay" transition={{ duration: 0.6, ease: "easeOut" }} />
                   )}
               </AnimatePresence>
          ...
          <button onClick={openBookingLink} className="w-full md:w-auto px-8 py-4 bg-white border border-gray-300 text-brand-dark font-bold text-lg rounded-full shadow-lg hover:bg-gray-50 transition-all hover:shadow-xl transform hover:-translate-y-1 flex items-center justify-center gap-2">
